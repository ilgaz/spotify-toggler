...
const parsePulseAudioData = data => {
  return data
    ...
    .map(data => {
      ...
      if (data.volume.includes('mono')) {
        const [_, volumeInfo] = data.volume.split('mono: ');
        volume = volumeInfo;
      } else {
      ...
      return {
        ...
        state: data.state,
        isMono: data.volume.includes('mono')
      };
    });
}
...
const evaluatePulseAudioLog = log => {
  const { volume, name, state, isMono } = log;

  if (isMono) {
    const [ _, percentage, decibel ] = volume.split('/').map(a => a.trim());
    return {
      left: {
        percentage,
        decibel
